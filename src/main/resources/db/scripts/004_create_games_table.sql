CREATE TABLE games (
    ...
    CONSTRAINT chk_games_players
        CHECK (min_players > 0 AND max_players >= min_players),
    
    CONSTRAINT chk_games_duration
        CHECK (avg_duration_minutes > 0),
    
    CONSTRAINT chk_games_difficulty
        CHECK (difficulty_level IN ('easy', 'medium', 'hard', 'expert')),
    
    CONSTRAINT chk_games_year
        CHECK (year_published > 1900 AND year_published <= EXTRACT(YEAR FROM CURRENT_DATE))
);
